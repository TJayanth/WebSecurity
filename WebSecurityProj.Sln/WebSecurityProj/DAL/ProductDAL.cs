...
namespace WebSecurityProj.DAL
{
    public class ProductDAL
    {
        string connectionString = "Data Source=JAYANTH-PC\\SQLSERVER; Initial Catalog=ProductMaster;User ID=sa;Password=India@123";

        public List<DeliveriesDTO> GetAllDeliveries(string wayBillNo)
        {
            List<DeliveriesDTO> lstStudent = new List<DeliveriesDTO>();
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                SqlCommand cmd = new SqlCommand(string.Format("SELECT * FROM Deliveries WHERE WayBillNo ={0}", wayBillNo), con);
                //SqlCommand cmd = new SqlCommand("SELECT * FROM Deliveries", con);
